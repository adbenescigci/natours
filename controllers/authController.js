...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  const activateToken = newUser.prepareActivateToken();
  ...
  try {
    await new Email(newUser, activateURL).sendActivate();
    ...
    console.log('erase that new account');
    return next(new AppError('There was error. Please try again', 500));
  }
});
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!!user.nonActivated)
    return next(new AppError('you should activate your account via mail', 400));
...
//Only for rendered pafes, no errors!
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  //1) Get user pased on POSTed email
  const user = await User.findOne({ email: req.body.email });
  if (!user) next(new AppError('There is no user with email address', 401));
